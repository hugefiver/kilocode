...
async function getTokenLimit(task: Task) {
	...
	return SIZE_LIMIT_AS_CONTEXT_WINDOW_FRACTION * modelInfo.contextWindow
	// kilocode_change end
}
...
export async function summarizeSuccessfulMcpOutputWhenTooLong(task: Task, outputText: string) {
	...
	const tokenLimit = await getTokenLimit(task) // kilocode_change: getTokenLimit is now async
